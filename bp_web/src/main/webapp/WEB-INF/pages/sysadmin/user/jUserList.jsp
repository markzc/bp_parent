...
${links}
	...
	<tr class="odd" onmouseover="this.className='highlight'" onmouseout="this.className='odd'" align="left">
		...
		<td>
		<c:if test="${o.userInfo.degree==0 }">超级管理员</c:if>
		<c:if test="${o.userInfo.degree==1 }">跨部门跨人员</c:if>
		<c:if test="${o.userInfo.degree==2 }">管理所有下属部门和人员</c:if>
		<c:if test="${o.userInfo.degree==3 }">管理本部门</c:if>
		<c:if test="${o.userInfo.degree==4 }">普通员工</c:if>
		</td>
		<td>${o.state }</td>
	</tr>
	</c:forEach>
